...
def create_calculator_tool(logger: ToolLogger):
    ...
    def solve_expression(expression: str) -> str:
        ...
        Args:
            expression: the expression that should be solved
        ...
        Returns:
            The original expression and the solution seperated by "="
        """
        if not isinstance(expression, str):
            error_msg = "Error: expression must be a string."
            logger.log_tool_use(
                "solve_expression",
        ...
        if not SAFE_MATH_PATTERN.match(expression):
            error_msg = "Error: Expression contains unsafe characters."
            logger.log_tool_use(
                "solve_expression",
                {"expression": expression},
                {"error": error_msg},
